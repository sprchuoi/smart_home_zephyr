...
int UartManager::init(struct k_msgq* msgq) {
    if (!msgq) {
    ...
    LOG_INF("UART manager initialized (interrupt-driven)");
    return 0;
}

void UartManager::uart_irq_handler(const struct device *dev, void *user_data) {
    UartManager *self = static_cast<UartManager*>(user_data);
...
int UartManager::send(const uint8_t* data, size_t len) {
